    ...
    <ul class="pagination pagination-sm no-margin pull-right">
        ...
        @if($data->lastPage() > 7)
            @if($data->currentPage() <= 3)
                @for($i = 1; $i <= ($data->currentPage()+1 > 3?$data->currentPage()+1:3); $i++)
                    <li @if($data->currentPage() == $i) class="active"@endif><a href="{{$data->url($i)/*route('admin.users').'?'.$data->pageName().'='.$i*/}}">{{$i}}</a></li>
                @endfor
            @else
                <li><a href="{{$data->url(1)}}">1</a></li>
                <li><a href="{{$data->url($data->currentPage()-2)}}">...</a></li>
                <li><a href="{{$data->url($data->currentPage()-1)}}">{{$data->currentPage()-1}}</a></li>
                <li class="active"><a href="{{$data->url($data->currentPage())}}">{{$data->currentPage()}}</a></li>
        ...
        @else
            @for($i = 1; $i <= $data->lastPage(); $i++)
                <li @if($data->currentPage() == $i) class="active"@endif><a href="{{$data->url($i)}}">{{$i}}</a></li>
            @endfor
        @endif
