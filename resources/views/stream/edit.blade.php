...
@section('content')
    ...
    @if($stream)
        <div class="row">
            <div class="col-md-12">
                <ul class="breadcrumb">
                    ...
                    <li>
                        <a href="#" class="active">/ Редактирование cтрим: {{$stream->title}}</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="content-box">
            <div class="col-md-12 section-title">
                <div>Редактирование cтрим</div>
            </div>
            <div class="row">
                <div class="col"></div>
                <div class="col-md-10">
                    <form action="{{route('stream.update',['id' => $stream->id])}}" enctype="multipart/form-data"
                        class="user-create-stream-form" method="post">
                        @csrf
                        <div class="form-fields-box">
                            <div class="form-group">
                                <label for="name">* Название:</label>
                                <input type="text" id="name" value="{{$stream->title??old('title')}}" name="title"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="custom-select {{ $errors->has('race') ? ' is-invalid' : '' }}"
                                            ...
                                            @foreach(\App\Replay::$races as $race)
                                                <option value="{{$race}}" {{($race == $stream->race|| $race == old('race'))?'selected':''}}>
                                                    {{$race}}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-select-2 custom-select {{ $errors->has('country_id') ? ' is-invalid' : '' }}"
                                            ...
                                            @foreach($countries as $country)
                                                <option
                                                        value="{{$country->id}}" {{($country->id == $stream->country_id || $country->id == old('country_id'))?'selected':''}}>
                                                    {{$country->name}}</option>
                                            @endforeach
                                        </select>
                    ...
                    </form><!-- close div /.user-create-stream-form -->
                </div>
                <div class="col"></div>
            </div><!-- close div /.row -->
    ...
    @else
        <div class="content-box">
            ...
            <div class="row">
                <div class="col"></div>
                <div class="col-10"><p class="margin-top-10 margin-bottom-10">У вас нет доступа к этому cтрим.</p></div>
                <div class="col"></div>
            </div>
        </div>
    @endif
